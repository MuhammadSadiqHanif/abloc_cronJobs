...
const demoTrading = require("../db/models/PaperTrading");
const demoWallet = require("../db/models/Demo_Wallet");
const { BuyAndSell } = require("./tradingFunctions");
const { DemoBuyAndSell } = require("./demotradingFunctions");
    ...
    FutureMarketTrading.find(
      {
        ...
        // currentMarketPrice: "0.3509",
        currentSelectedCoin: obj.symbol,
        // currentSelectedCoin: "ALGO",
        positionStatus: "open",
      },
      async (err, orders) => {
        if (err) return;
...
const demoFutureTradingRequestFunction = async (values, io) => {
  //console.log(obj);
  let data = {
    fromSymbol: values.currentSelectedCoin,
    fromAmount: values.currentMarketPrice,
  ...
  if (data.userId) {
    if (
      ...
      demoWallet.find({ userId: data.userId }, async (err, docs) => {
        //console.log(docs);
        if (!err && docs.length) {
          let getdata = { message: "Success", data: docs[0]._doc };
          let obj = DemoBuyAndSell(
            getdata.data,
            {
              fromSymbol: data.fromSymbol,
              fromAmount: data.fromAmount,
